...
local function styleCollections()
	...
	KS:CreateBDFrame(WardrobeOutfitFrame, .25)
	KS:CreateSD(WardrobeOutfitFrame, .25)
	...
	if E.db.KlixUI.misc.transmog then	
		...
		local button = CreateFrame("Button", KUI.Title.."TransmogRemoveButton", WardrobeTransmogFrame, "UIMenuButtonStretchTemplate")
		...
		button:SetPoint(unpack(defaultPoint))
		...
		ClearAllPendingButton:HookScript("OnShow", function(self)
			button:SetPoint(unpack(pendingPoint))
		end)
...
function S:RemoveAllTransmog()
	for _, slot in pairs(TransmogSlots) do
...
function S:RemoveTransmog(slotID)
  local isTransmogrified, hasPending = C_Transmog.GetSlotInfo(slotID, LE_TRANSMOG_TYPE_APPEARANCE)
