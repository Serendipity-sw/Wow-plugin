...
-- GLOBALS: MerchantFrame, GameTooltip, MERCHANT_ITEMS_PER_PAGE, LE_ITEM_CLASS_WEAPON, LE_ITEM_CLASS_ARMOR, CreateFrame, MERCHANT_ITEMS_PER_PAGE, BUYBACK_ITEMS_PER_PAGE, hooksecurefunc, MERCHANT_PAGE_NUMBER, UIParent, ChatFontSmall
...
local function MerchantItemlevel()
	if E.db.KlixUI.misc.merchant.itemlevel ~= true then return end
	...
	for i = 1, MERCHANT_ITEMS_PER_PAGE do
		...
		if button and button:IsShown() then
			if not button.text then
				button.text = KUI:CreateText(button, "OVERLAY", 12, "OUTLINE")
...
local function MerchantSkinInit()
	...
	UpdateButtonsPositions()
	CreateFrame("GameTooltip", "KUI_Merchant_HiddenTooltip", UIParent, "GameTooltipTemplate");
