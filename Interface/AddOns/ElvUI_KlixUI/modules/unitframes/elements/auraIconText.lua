...
local function UpdateAuraIconSettings(self, auras, noCycle)
	...
	local index = 1
	if(db) then
		local config = E.db.KlixUI.unitframes.AuraIconText
		if(not noCycle) then
...
local function UpdateAuraTimer(self, elapsed)
	local hideDurationText = E.db.KlixUI.unitframes.AuraIconText.hideDurationText
	local hideStackText = E.db.KlixUI.unitframes.AuraIconText.hideStackText
	local durationThreshold = E.db.KlixUI.unitframes.AuraIconText.durationThreshold
	local durationFilterOwner = E.db.KlixUI.unitframes.AuraIconText.durationFilterOwner
	local stackFilterOwner = E.db.KlixUI.unitframes.AuraIconText.stackFilterOwner
	local showText = true
