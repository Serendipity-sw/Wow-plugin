...
local enhancedClickMenu = function(self, button)
	...
	if button == "RightButton" and IsAltKeyDown() and IsControlKeyDown() then
		...
		EasyMenu(menu, menuFrame, menuDatatext, 0 , 3, "MENU", 2);			
	else
...
function KDT:Initialize()
	...
	menuFrame = CreateFrame("Frame", "KDTMenuFrame", E.UIParent, "UIDropDownMenuTemplate")
	...
	hooksecurefunc(DT, "LoadDataTexts", function()
		...
		for name, _ in pairs(DT.RegisteredDataTexts) do
			tinsert(menu, { text = name, func = function() KDT:ChangeDatatext(name) end, checked = false })
		end
		tsort(menu, function(a,b) return a.text < b.text end)
		tinsert(menu, 1, { text = 'None', func = function() KDT:ChangeDatatext('') end, checked = false })
