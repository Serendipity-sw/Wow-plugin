...
local function Update(self, t)
	...
	if int2 <= 0 then
		...
		local latency = select(E.db.KlixUI.systemDT.latency == "world" and 4 or 3, GetNetStats())
...
local function Click(self, button)
	if button == "LeftButton" and not InCombatLockdown() then
		...
		if E.db.KlixUI.systemDT.announceFreed then
			KUI:Print(format(L["Garbage Collection Freed"]..(" |cff00ff00%s|r"):format(FormatMemory(preCollect - postCollect))))
		end
