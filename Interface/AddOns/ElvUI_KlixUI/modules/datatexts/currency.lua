...
local GetMoney, GetCurrencyInfo, GetNumWatchedTokens, GetBackpackCurrencyInfo, GetCurrencyListInfo, GetCurrencyListSize, GetProfessions, GetProfessionInfo = GetMoney, GetCurrencyInfo, GetNumWatchedTokens, GetBackpackCurrencyInfo, GetCurrencyListInfo, GetCurrencyListSize, GetProfessions, GetProfessionInfo
...
local resetInfoFormatter = join("", "|cffaaaaaa", L["Hold Shift + Right Click: Reset Data"], "|r")
...
local function OnEnter(self)
	...
	if GetOption("BFA") then
		GetCurrency(BattleForAzertoh, format("%s:", L["Battle For Azeroth"]))
	end
	if GetOption("LEGION") then
		GetCurrency(Legion, format("%s:", L["Legion"]))
	end
	if GetOption("WOD") then
		GetCurrency(WarlordsOfDraenor, format("%s:", L["Warlords of Draenor"]))
	end
	if GetOption("MOP") then
		GetCurrency(MistsOfPandaria, format("%s:", L["Mists of Pandaria"]))
	end
	if GetOption("CATA") then
		GetCurrency(Cataclysm, format("%s:", L["Cataclysm"]))
	end
	if GetOption("WOLK") then
		GetCurrency(WrathOfTheLichKing, format("%s:", L["Wrath of the Lich King"]))
	end
	
	ToggleCurrencies(false)
	
	if E.db.KlixUI.currencyDT.WoWToken then
		local Token = C_WowTokenPublic.GetCurrentMarketPrice()
