	...
	if(UnitIsPlayer(unit)) then
		...
		if(realm and realm ~= "") then
			if(isShiftKeyDown) or self.db.alwaysShowRealm then
				name = name..format("|cfff960d9%s|r", " - "..realm)
			elseif(relationship == LE_REALM_RELATION_COALESCED) then
				name = name..format("|cfff960d9%s|r", FOREIGN_SERVER_LABEL)
			elseif(relationship == LE_REALM_RELATION_VIRTUAL) then
				name = name..format("|cfff960d9%s|r", INTERACTIVE_SERVER_LABEL)
			end
		end
		...
		if(guildName) then
			...
			if(self.db.guildRanks) then
				GameTooltipTextLeft2:SetText(("|cfff960d9[|r|cff00ff10%s|r|cfff960d9]|r <|cff00ff10%s|r>"):format(guildName, guildRankName))
			else
