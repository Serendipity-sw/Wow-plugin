...
function KA:UpdatePaperDoll(inspect)
  if not E.db.KlixUI.armory.enable then return end

  if InCombatLockdown() then
		KA:RegisterEvent("PLAYER_REGEN_ENABLED", "UpdatePaperDoll", inspect)
		return
  else
		KA:UnregisterEvent("PLAYER_REGEN_ENABLED")
  end
  ...
  local avgItemLevel, avgEquipItemLevel = GetAverageItemLevel()
	...
	for k, info in pairs(slots) do
		...
		if info[1] then
			...
			if KA.db.ilvl.enable and info[1] then
...
function KA:DelayUpdateInfo(inspect)
	if (updateTimer == 0 or KA:TimeLeft(updateTimer) == 0) then
		updateTimer = KA:ScheduleTimer("UpdatePaperDoll", .2, inspect)
	end
end
...
function KA:InspectFrame_UpdateTabsComplete()
	originalInspectFrameUpdateTabs()
	KA:DelayUpdateInfo(true)
end
...
function KA:InitialUpdatePaperDoll()
	...
	_G.InspectFrame_UpdateTabs = KA.InspectFrame_UpdateTabsComplete
...
function KA:BuildInfoText(name)
	for k, info in pairs(slots) do
		...
		if info[1] then
			...
			frame.ItemLevel:FontTemplate(E.LSM:Fetch("font", KA.db.ilvl.font), KA.db.ilvl.textSize, KA.db.ilvl.fontOutline)
		end

		if name == "Character" and info[2] then
			frame.DurabilityInfo = frame:CreateFontString(nil, "OVERLAY")
			frame.DurabilityInfo:SetPoint("TOP", frame, "TOP", 0, -4)
			frame.DurabilityInfo:FontTemplate(E.LSM:Fetch("font", KA.db.durability.font), KA.db.durability.textSize, KA.db.durability.fontOutline)
		end
	end
...
function KA:UpdateIlvlFont()
	local db = E.db.KlixUI.armory.stats.ItemLevel
...
function KA:Initialize()
	if not E.db.KlixUI.armory.enable or IsAddOnLoaded('ElvUI_SLE') then return end
	...
	if not KUI._Compatibility["DejaCharacterStats"] then
