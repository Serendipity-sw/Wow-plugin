...
function KBF:BonusRollFrame_OnShow(frame)
    ...
    if (KBF_RollFrame == nil) then
        KBF_RollFrame = frame
    elseif (KBF_RollFrameDifficultyIdBackup ~= nil and KBF_RollFrameEndTimeBackup ~= nil and frame.difficultyID ~= KBF_RollFrameDifficultyIdBackup and time() <= KBF_RollFrameEndTimeBackup) then
        KBF_RollFrame = frame
    ...
    if (KBF_RollFrame.difficultyID == 8) then
        ...
        elseif (E.db.KlixUI.loot.bonusFilter.disableKeystoneLevelToggle == true and KBF_ShowBonusRoll == false) then
            ...
            if (level < tonumber(E.db.KlixUI.loot.bonusFilter.disableKeystoneLevel)) then
                KBF:HideRoll()
            end
        end
...
function KBF:HideRoll()
    KBF_RollFrame:Hide()
    GroupLootContainer_RemoveFrame(GroupLootContainer, KBF_RollFrame);
	KUI:Print("Bonus Roll hidden")
end

function KBF:ShowRoll()
    if KBF_RollFrame ~= nil and (KBF_RollFrame:IsVisible() == false and time() <= KBF_RollFrame.endTime and KBF_UserAction == false) then
        KBF_ShowBonusRoll = true
        GroupLootContainer_AddFrame(GroupLootContainer, KBF_RollFrame);
