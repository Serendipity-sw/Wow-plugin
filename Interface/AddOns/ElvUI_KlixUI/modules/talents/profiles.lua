...
function KBTP:GetProfile(index)
    return KUIDataDB[self.playerClass].specs[GetSpecialization()].profiles[index]
end

-- Returns a list of all profiles for the current spec
function KBTP:GetAllProfiles()
    return KUIDataDB[self.playerClass].specs[GetSpecialization()].profiles
end

-- Inserts a new profile into the current spec's DB
function KBTP:InsertProfile(profile)
    table.insert(KUIDataDB[self.playerClass].specs[GetSpecialization()].profiles, profile)
end

-- Removes the profile at the given index from the current spec's DB
function KBTP:RemoveProfile(index)
    table.remove(KUIDataDB[self.playerClass].specs[GetSpecialization()].profiles, index)
end
...
StaticPopupDialogs["TALENTPROFILES_REMOVE_PROFILE"] = {
    ...
    OnAccept = function(sender)
        local key = nil
        local i = 1
        for k, v in pairs(KBTP:GetProfile(TalentProfiles_profilesDropDown.selectedID)) do
...
function KBTP:BuildFrame()
    ...
    local enabled = KBT.selectedSpec == GetSpecialization()
