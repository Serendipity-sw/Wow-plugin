...
AP.modName = L["Addon Control Panel"]
...
function AP:BuildFrame()
	local Frame = CreateFrame("Frame", "APFrame", UIParent)
	local Close = CreateFrame("Button", "APCloseButton", Frame)
	local Reload = CreateFrame("Button", "APReload", Frame)
	local Search = CreateFrame("EditBox", "APSearchBox", Frame)
	local CharacterSelect = CreateFrame("Button", nil, Frame)
	local Profiles = CreateFrame("Button", "APProfiles", Frame)
	local AddOns = CreateFrame("Frame", "APAddOns", Frame)
	...
	Close:SetSize(16, 16)
	Close:SetScript('OnEnter', function(self) self:SetBackdropBorderColor(unpack(AP.db['ClassColor'] and AP.ClassColor or AP.db['CheckColor'])) end)
	...
	Mask:SetTexture([[Interface\AddOns\ElvUI\media\textures\close]], 'CLAMPTOBLACKADDITIVE', 'CLAMPTOBLACKADDITIVE')
	Mask:SetSize(10, 10)
	Mask:SetPoint('CENTER')

	Close.Mask = Mask

	Close:SetNormalTexture(LSM:Fetch('statusbar', AP.db['CheckTexture']))
	Close:SetPushedTexture(LSM:Fetch('statusbar', AP.db['CheckTexture']))
	...
	Search:HookScript('OnEditFocusGained', function(self) self:SetBackdropBorderColor(unpack(AP.db['ClassColor'] and AP.ClassColor or AP.db['CheckColor'])) self:HighlightText() end)
	...
	CharacterSelect.DropDown = CreateFrame('Frame', 'APCharacterSelectDropDown', CharacterSelect, 'UIDropDownMenuTemplate')
	...
	CharacterSelect:SetScript('OnClick', function(self) EasyMenu(AP.Menu, self.DropDown, self, 0, 38 + (AP.MenuOffset * 16), "MENU", 5) end)
	...
	Profiles:SetScript('OnEnter', function(self) self:SetBackdropBorderColor(unpack(AP.db['ClassColor'] and AP.ClassColor or AP.db['CheckColor'])) end)
	...
	Thumb:Width(10)
	Thumb:Height(16)
	Thumb:SetVertexColor(unpack(AP.db['ClassColor'] and AP.ClassColor or AP.db['CheckColor']))
	...
	for i = 1, 30 do
		local CheckButton = CreateFrame('CheckButton', 'APCheckButton_'..i, AddOns)
		...
		CheckButton:SetScript('OnClick', function(self)
			if self.name then
				if KUI:IsAddOnEnabled(self.name, AP.SelectedCharacter) then
					DisableAddOn(self.name, AP.SelectedCharacter)
				else
					EnableAddOn(self.name, AP.SelectedCharacter)
				end
				AP:UpdateAddonList()
			end
		end)
		...
		Checked:SetVertexColor(unpack(AP.db['ClassColor'] and AP.ClassColor or AP.db['CheckColor']))
...
function AP:NewAddOnProfile(name, overwrite)
	if AddonPanelProfilesDB[name] and (not overwrite) then
		StaticPopupDialogs['ADDONPANEL_OVERWRITEPROFILE'].text = format(L['There is already a profile named %s. Do you want to overwrite it?'], name)
	...
	for i = 1, #self.AddOnInfo do
		local AddOn, isEnabled = unpack(self.AddOnInfo[i]), KUI:IsAddOnEnabled(i, AP.SelectedCharacter)
		if isEnabled then
			tinsert(AddonPanelProfilesDB[name], AddOn)
		end
	end
...
function AP:InitProfiles()
	local ProfileMenu = CreateFrame('Frame', 'APProfileMenu', self.Frame)
	...
	ProfileMenu.EnableAll:SetScript('OnClick', function(self)
		EnableAllAddOns(AP.SelectedCharacter)
		AP:UpdateAddonList()
	end)
	...
	ProfileMenu.DisableAll:SetScript('OnClick', function(self)
		DisableAllAddOns(AP.SelectedCharacter)
		AP:UpdateAddonList()
	end)
	...
	for i = 1, 10 do
		...
		Pullout.Load:SetScript('OnClick', function(self, btn)
			if btn == 'RightButton' then
				...
				for _, AddOn in pairs(AddonPanelProfilesDB[Pullout.Name]) do
					EnableAddOn(AddOn, AP.SelectedCharacter)
				end
		...
		Pullout.Delete:SetScript('OnClick', function(self)
			...
			dialog.text = format(L['Are you sure you want to delete %s?'], Pullout.Name)
			dialog.OnAccept = function(self)
				AddonPanelProfilesDB[Pullout.Name] = nil
				AP:UpdateProfiles()
			end
...
function AP:UpdateProfiles()
	...
	for name, _ in pairs(AddonPanelProfilesDB) do
...
function AP:UpdateAddonList()
	...
	for i = 1, AP.db['NumAddOns'] do
		...
		if addonIndex and addonIndex <= #self.AddOnInfo then
			...
			button.CheckTexture:SetVertexColor(unpack(KUI:IsAddOnPartiallyEnabled(addonIndex, AP.SelectedCharacter) and {.6, .6, .6} or AP.db['ClassColor'] and AP.ClassColor or AP.db['CheckColor']))
			button:Show()
		else
			button:Hide()
		end
...
function AP:Update()
	for i = 1, 30 do
		...
		CheckButton.CheckTexture:SetVertexColor(unpack(AP.db['ClassColor'] and AP.ClassColor or AP.db['CheckColor']))
	...
	if AP.db["FontColor"] == 1 then
		self.Frame.Search:SetTextColor(KUI.ClassColor.r, KUI.ClassColor.g, KUI.ClassColor.b)
		self.Frame.Reload.Text:SetTextColor(KUI.ClassColor.r, KUI.ClassColor.g, KUI.ClassColor.b)
		self.Frame.CharacterSelect.Text:SetTextColor(KUI.ClassColor.r, KUI.ClassColor.g, KUI.ClassColor.b)
		self.Frame.Profiles.Text:SetTextColor(KUI.ClassColor.r, KUI.ClassColor.g, KUI.ClassColor.b)
		self.ProfileMenu.EnableAll.Text:SetTextColor(KUI.ClassColor.r, KUI.ClassColor.g, KUI.ClassColor.b)
		self.ProfileMenu.DisableAll.Text:SetTextColor(KUI.ClassColor.r, KUI.ClassColor.g, KUI.ClassColor.b)
		self.ProfileMenu.NewButton.Text:SetTextColor(KUI.ClassColor.r, KUI.ClassColor.g, KUI.ClassColor.b)
		for i = 1, 10 do
			self.ProfileMenu.Buttons[i].Load.Text:SetTextColor(KUI.ClassColor.r, KUI.ClassColor.g, KUI.ClassColor.b)
			self.ProfileMenu.Buttons[i].Update.Text:SetTextColor(KUI.ClassColor.r, KUI.ClassColor.g, KUI.ClassColor.b)
			self.ProfileMenu.Buttons[i].Delete.Text:SetTextColor(KUI.ClassColor.r, KUI.ClassColor.g, KUI.ClassColor.b)
		end
...
function AP:Initialize()
	if not E.db.KlixUI.addonpanel.Enable or (IsAddOnLoaded("ProjectAzilroka") and _G.ProjectAzilroka.db["stAddonManager"]) then return end
	...
	AP.SelectedCharacter = UnitName('player')
	...
	AddonPanelServerDB[GetRealmName()] = AddonPanelServerDB[GetRealmName()] or {}
	AddonPanelServerDB[GetRealmName()][UnitName('player')] = true

	local index = 2
	for Character in KUI:PairsByKeys(AddonPanelServerDB[GetRealmName()]) do
