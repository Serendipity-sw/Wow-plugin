...
CH.KUIUpdateAnchors = CH.UpdateAnchors
function CH:UpdateAnchors()
	self:KUIUpdateAnchors()
			...
			if selectedId == tab:GetID() and E.db.KlixUI.chat.select then
				if tab.hasBracket ~= true or isForced then
					...
					if E.db.KlixUI.chat.styleTab == "DEFAULT" or E.db.KlixUI.chat.styleTab == "SQUARE" or E.db.KlixUI.chat.styleTab == "BEND" then
						tab.text:SetText(format(KC.SelectedStrings[E.db.KlixUI.chat.styleTab], color.r * 255, color.g * 255, color.b * 255, (FCF_GetChatWindowInfo(tab:GetID())), color.r * 255, color.g * 255, color.b * 255))
					elseif E.db.KlixUI.chat.styleTab == "HALFDEFAULT" then
						tab.text:SetText(format(KC.SelectedStrings[E.db.KlixUI.chat.styleTab], color.r * 255, color.g * 255, color.b * 255, (FCF_GetChatWindowInfo(tab:GetID()))))
					else
						tab.text:SetText(format(KC.SelectedStrings[E.db.KlixUI.chat.styleTab], (E.db.chat.tabFontSize + 12), (FCF_GetChatWindowInfo(tab:GetID()))))
					end
					tab.hasBracket = true
				end
			else
		...
		if self.isDocked and GeneralDockManager.selected:GetID() == self:GetID() and not self.isTemporary and E.db.KlixUI.chat.select then
			local color = E.db.KlixUI.chat.colorTab
				if E.db.KlixUI.chat.styleTab == "DEFAULT" or E.db.KlixUI.chat.styleTab == "SQUARE" or E.db.KlixUI.chat.styleTab == "BEND" then
					tab.text:SetText(format(KC.SelectedStrings[E.db.KlixUI.chat.styleTab], color.r * 255, color.g * 255, color.b * 255, (FCF_GetChatWindowInfo(tab:GetID())), color.r * 255, color.g * 255, color.b * 255))
				elseif E.db.KlixUI.chat.styleTab == "HALFDEFAULT" then
					tab.text:SetText(format(KC.SelectedStrings[C.db.tab.style], color.r * 255, color.g * 255, color.b * 255, (FCF_GetChatWindowInfo(tab:GetID()))))
				else
					tab.text:SetText(format(KC.SelectedStrings[E.db.KlixUI.chat.styleTab], (E.db.chat.tabFontSize + 12), (FCF_GetChatWindowInfo(tab:GetID()))))
				end
