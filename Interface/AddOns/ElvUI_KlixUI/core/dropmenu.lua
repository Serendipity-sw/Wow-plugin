-- Credits go to benik for this mod.
...
function KUI:Dropmenu(list, frame, parent, pos, xOffset, yOffset, delay, addedSize)
	...
	for i=1, #list do 
		...
		elseif db.color == 2 then
			frame.buttons[i].text:SetTextColor(KUI:unpackColor(db.customColor))
		else
			frame.buttons[i].text:SetTextColor(KUI:unpackColor(E.db.general.valuecolor))
		end
