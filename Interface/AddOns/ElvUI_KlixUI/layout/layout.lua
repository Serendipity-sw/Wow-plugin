...
local Kui_ldtp = CreateFrame('Frame', 'KuiLeftChatDTPanel', E.UIParent)
local Kui_rdtp = CreateFrame('Frame', 'KuiRightChatDTPanel', E.UIParent)
local Kui_mdtp = CreateFrame('Frame', 'KuiMiddleDTPanel', E.UIParent)
...
local function RegKuiDataTexts()
	...
	DT:RegisterPanel(KuiMiddleDTPanel, 3, 'ANCHOR_BOTTOM', 0, -4)
...
function KUIL:CreateExtraDataBarPanels()
	local leftchattab = CreateFrame("Frame", "Left_ChatTab_Panel", LeftChatPanel)
	...
	local rightchattab = CreateFrame("Frame", "Right_ChatTab_Panel", RightChatPanel)
...
local Kui_dchat = CreateFrame('Frame', 'KuiDummyChat', E.UIParent)
local Kui_dthreat = CreateFrame('Frame', 'KuiDummyThreat', E.UIParent)

local menuFrame = CreateFrame('Frame', 'KuiGameClickMenu', E.UIParent)
...
local function UpdateBar4(self, bar)
	GameTooltip:Hide()
	if InCombatLockdown() then KUI:Print(ERR_NOT_IN_COMBAT) return end
	local button = self
...
function KUIL:ChangeLayout()
	...
	-- Buttons
	for i = 1, BUTTON_NUM do
		kbuttons[i] = CreateFrame('Button', 'KuiButton_'..i, E.UIParent)
		...
		elseif i == 2 then
			...
			kbuttons[i]:SetScript('OnEnter', function(self)
				...
				GameTooltip:Show()
				if InCombatLockdown() or KuiGameClickMenu:IsShown() then GameTooltip:Hide() end
			end)
		...
		elseif i == 3 then
			...
			kbuttons[i]:SetScript('OnEnter', function(self)
				...
				self:SetScript('OnClick', function(self, btn)
				if IsControlKeyDown() and E.db.KlixUI.actionbars.SEBar.enable then
				...
				GameTooltip:ClearLines()
				if E.db.KlixUI.chat.voicemenu == true then
					GameTooltip:AddLine(L['Left Click: Toggle Voice Chat'], selectioncolor)
				end
				GameTooltip:AddLine(L['Right Click: Toggle Addon List'], selectioncolor)
				GameTooltip:AddLine(L['Shift + Click: Toggle Chat'], 0.7, 0.7, 1)
		...
		elseif i == 4 then
			...
			kbuttons[i]:SetScript('OnEnter', function(self)
				...
				self:SetScript('OnClick', function(self, btn)
				if IsControlKeyDown() and E.db.KlixUI.datatexts.middle.enable == true then
...
function KUIL:ShadowOverlay()
	...
	if E.db.KlixUI.general.shadowOverlay ~= true then return end

	self.f = CreateFrame("Frame", KUI.Title.."ShadowBackground")
...
-- Credits: Merathilis
function KUIL:ChatButtonHolder()
	...
	if ChatButtonHolder then
		...
		ChatButtonHolder:SetPoint("LEFT", LeftChatPanel, "RIGHT", 2, -12)
		ChatButtonHolder:SetSize(27, LeftChatPanel:GetHeight()+21)

		ChatButtonHolder.bg = KS:CreateBDFrame(ChatButtonHolder, .60)
		ChatButtonHolder.bg:Styling(false, false, false, 8, 8, 1)
		
		E:DisableMover("SocialMenuMover")
		if E.db.KlixUI.chat.voicemenu ~= true then
			ChatButtonHolder:Kill()
		end
	end
end
hooksecurefunc(LO, "CreateChatButtonPanel", KUIL.ChatButtonHolder)
